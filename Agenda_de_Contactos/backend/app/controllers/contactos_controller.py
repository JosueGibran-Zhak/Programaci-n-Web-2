...
from app.models.contacto_model import Contacto, ContactoCreate, CategoriaLista
                                            #CategoriaLista se usa para que reconozca la lista de categorías
...
def crear_contacto(datos: ContactoCreate) -> Contacto:
    ...
    nuevo_contacto = Contacto(
        ...
        correo=datos.correo,
        #Añadiendo la categoria para que el backend lo cree
...
#Función para obtener los contactos por categoría.
def obtener_contactos_por_cat(categoria: CategoriaLista) -> list[Contacto]:
    #list[Contacto] servirá para pasarle la lista de contactos en vez de pasarle solo un contacto.
    """
    Parámetros:
    -categoría: cada elemento será de CategoriaLista, el cual tiene 3 valores posibles de tipo string
    
    *Retorna el contacto encontrado
    *Si no existe lanza una excepción HTTP 400
    """
    contactos_filtrados = []
#-Busca en toda la "Base de datos", los contactos que coincidan con la categoría
#-Después los agrega a una lista nueva de contactos filtrados
    for contacto in contactos_db:
        if contacto.categoria == categoria:
            contactos_filtrados.append(contacto) #Agrega un contacto nuevo a la lista filtrada

    if not contactos_filtrados: #Si la lista está vacía lanza un error 
        raise HTTPException(
            status_code = 400,
            detail = "Contacto no encontrado"
        )
