...
import { Contacto, ContactoCreate,Categoria } from '../models/contacto';

//ESTE SERVICIO SOLO VA A SER PARA CONTACTOS.
...
export class ContactosService {
   ...
   */
  private contactosSignal = signal<Contacto[]>([]); //Hace una actualizacion automática.
  ...
  //Obtiene la categoria con un signal
  private obtenerCategoriaSignal =signal<Categoria>(Categoria.familia);
  categoria = this.obtenerCategoriaSignal.asReadonly();
   ...
   *
   * Es decir: No guarda un estado independiente; en lugar de eso,
  ...
  const texto = this.textoBusquedaSignal().toLowerCase();
  const categoria = this.obtenerCategoriaSignal();

  //Retorna los contactos filtrados
  return this.contactosSignal().filter(contacto => {
    ...
    const coincideCategoria =
      !categoria || contacto.categoria === categoria;
  ...
  obtenerPorCategoria(categoria: Categoria): void {
  this.cargandoSignal.set(true);
  this.errorSignal.set('');

  this.http.get<Contacto[]>(`${this.apiUrl}/categoria/${categoria}`)
    .subscribe({
      next: (respuesta) => {
        this.contactosSignal.set(respuesta);
        this.cargandoSignal.set(false);
  ...
  actualizarCategoria(categoria: Categoria) {
    this.obtenerCategoriaSignal.set(categoria);
  }
//  }
  /**
   * ----------------------------------------------------------
